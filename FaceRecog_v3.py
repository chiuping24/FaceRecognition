...
def load_data(dataset_path):
    ...
    faces = np.empty((400, 57 * 47))   #array[400, 57*47]
    ...
    label = np.zeros((400, 40)) # tag the people correct to 1, initial all label is 0
    for i in range(40):
        label[i * 10: (i + 1) * 10, i] = 1 # [00~09, label_0]=1,  [10~19, label_1]=1, [20~29, label_2]=1,.. 
    ...
    for i in range(40):
        train_data[i * 8: i * 8 + 8] = faces[i * 10: i * 10 + 8] #each man pre-8 photos are as trainning 
        train_label[i * 8: i * 8 + 8] = label[i * 10: i * 10 + 8]

        vaild_data[i] = faces[i * 10 + 8] #each man 9th photo is as valid 
        vaild_label[i] = label[i * 10 + 8]

        test_data[i] = faces[i * 10 + 9] #each man 10th photo is as testing 
...
def convolutional_layer(data, kernel_size, bias_size, pooling_size):
    ...
    conv = tf.nn.conv2d(data, kernel, strides=[1, 1, 1, 1], padding='SAME', name = "conv2d")
    linear_output = tf.nn.relu(tf.add(conv, bias), name = "relu")
    pooling = tf.nn.max_pool(linear_output, ksize=pooling_size, strides=pooling_size, padding="SAME", name ="pool")
...
def convolutional_neural_network(data):
    ...
    kernel_shape1=[5, 5, 1, 32]  #layer1  input 1, output 32
    kernel_shape2=[5, 5, 32, 64] #layer2  input 32, output 64
    #kernel_shape3=[5, 5, 64, 128] ##
    full_conn_w_shape = [15 * 12 * 64, 1024]  # 8*6 *128  for 3 layers
    
    out_w_shape = [1024, n_ouput_layer]  #input full-connect 1024, output 40 kinds of man label
    ...
    ## 经过第3层卷积神经网络后，得到的张量shape为：[batch, 8, 6, 128]
  ...
  #      )
    ...
    with tf.variable_scope("full_connection") as full_layer3:
        # 讲卷积层张量数据拉成2-D张量只有有一列的列向量
        layer3_output_flatten = tf.contrib.layers.flatten(layer2_output)  ##!! layer2_output: 2 layer conv
...
def train():
    ...
    X = tf.placeholder(tf.float32, [batch_size, 57 * 47], name = "input")
    Y = tf.placeholder(tf.float32, [batch_size, 40], name = "label")
    #Y = tf.placeholder(tf.float32, [batch_size, 20])

    predict = convolutional_neural_network(X)
    cost = tf.nn.softmax(logits=predict)
    cost_func = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(logits=predict, labels=Y), name="cost")
    ...
    f = open("ModelInfo.txt", "w")  # write a model information into txt file
    with tf.Session() as session:
        ...
        if not os.path.exists(model_path + ".index"):
            ...
            for epoch in range(17):
                ...
                for i in range(int(np.shape(train_set_x)[0] / batch_size)):
                    ...
                    print('!!!!')
                    #print(session.run(cost, feed_dict={X: x, Y: y})) ####
                    #print(session.run(cost, feed_dict={X: x, Y: y})[2])
                    #print(session.run(cost, feed_dict={X: x, Y: y})[3])
        ...
        #print('!!!!!')##
        #print(session.run(cost)) #check the results of softmax
        ...
        f.write("valid set accuracy: ")
        f.write(str(valid_accuracy.eval({X: vaild_set_x, Y: valid_set_y})))
        f.write("\n")
        #print(predict({X: test_set_x}))
        ...
        #print(session.run(tf.Variable(tf.constant(1.0, shape=[1]), name="v1")))
        #print("~~~all test:")
        #_index = [i for i in range(np.shape(test_correct)[0])]
        #for i in _index:
        #    print('picture person is %i, predicted as person %i'
        #        %(test_true[i], test_pred[i]))
            #f.write("Wrong prediction: ")
        #    f.write(str(test_true[i]))
        #    f.write(", predicted as persion ")
